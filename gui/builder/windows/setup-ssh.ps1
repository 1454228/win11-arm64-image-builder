...
if ($sshInstalled) {
    ...
    # An administrator account key is not read from the home directory but from administrators_authorized_keys,
    # and the ACL must contain only SYSTEM+Administrators
    ...
    # Ensure port 22 is allowed (MSI usually ships its own rule; use a separate name to avoid conflicts)
